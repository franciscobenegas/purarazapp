...
type SalidaItemForm = { id: string; categoriaId: string; cantidad: number };
  ...
  const form = useForm<z.infer<typeof SalidaSchema>>({
    ...
    defaultValues: {
      fecha: salida.fecha
        ...
        : "",
      propietarioId: salida.propietarioId,
      motivoId: salida.motivoId,
      NombreEstanciaSalida: salida.NombreEstanciaSalida || "",
    },
  });
...
useEffect(() => {
  if (salida) {
    form.reset({
      fecha: salida.fecha
        ? new Date(salida.fecha).toISOString().slice(0, 10)
        : "",
      propietarioId: salida.propietarioId,
      motivoId: salida.motivoId,
      NombreEstanciaSalida: salida.NombreEstanciaSalida?.trim() || "",
    });
  ...
  const onSubmit = async (values: z.infer<typeof SalidaSchema>) => {
    ...
    try {
      ...
      if (resp.ok) {
        toast.success("Éxito", { description: "Salida actualizada." });
        router.refresh();
      } else {
  ...
  return (
    <div className="min-h-screen bg-gradient-to-br p-2 md:p-2 ">
      <div className="mx-auto max-w-4xl">
        <Card>
          <CardHeader>
            <CardTitle>Editar Salida</CardTitle>
          </CardHeader>
          <CardContent>
            <Form {...form}>
              <form
              ...
              >
                <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                  ...
                  <FormField
                    ...
                    render={({ field }) => (
                      <FormItem>
                        <FormLabel>Estancia Salida</FormLabel>
                        <FormControl>
                          <Input placeholder="Estancia Salida" {...field} />
                        </FormControl>
                        <FormMessage />
                      </FormItem>
                    )}
                ...
                <div className="flex gap-2">
                  ...
                  <Button
                    type="button"
                    variant="secondary"
                    onClick={() => router.push("/diaria/salida")}
                    disabled={loading}
                  >
                    Cancelar
                  </Button>
