...
export function FormViewMortandadId({ mortandad }: FormMortandadProps) {
  ...
  const openGoogleMaps = () => {
    // Verificar si la ubicación GPS parece ser coordenadas
    ...
    if (isLikelyCoordinates) {
      // Si son coordenadas, usarlas directamente
      window.open(
        `https://www.google.com/maps?q=${mortandad.ubicacionGps}`,
        "_blank"
      );
    } else {
      // Si no son coordenadas, tratar como dirección textual
      window.open(
  ...
  return (
    <div>
      <Card className="max-w-3xl mx-auto shadow-md rounded-xl border mt-5">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-2">
              ...
              <div>
                ...
                <div className="flex gap-1 mt-1">
                  <Badge className="text-xs">{mortandad.categoria.sexo}</Badge>
                  <Badge className="text-xs">{mortandad.categoria.edad}</Badge>
                </div>
              </div>
            </div>
          </div>
