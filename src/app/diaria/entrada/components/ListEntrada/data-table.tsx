...
import { Entrada,  Prisma } from "@prisma/client";
...
interface DataTableProps {
  data: EntradaWithRelations[];
}

type DateRangeFilter = { from?: string; to?: string };

export function DataTableEntrada({ data }: DataTableProps) {
  ...
  const [deletingEntrada, setDeletingEntrada] = useState<
  ...
  const handleDeleteConfirm = async () => {
    if (deletingEntrada) {
      setLoading(true);
      try {
        const ids = Array.isArray(deletingEntrada)
          ? deletingEntrada.map((item) => item.id)
          : [deletingEntrada?.id];
  ...
  const columns: ColumnDef<EntradaWithRelations>[] = [
    {
      id: "select",
      header: ({ table }) => (
    ...
    {
      accessorKey: "NombreEstanciaOrigen",
        ...
        return (
          <DropdownMenu>
            ...
            <DropdownMenuContent align="end">
              ...
              <DropdownMenuSeparator />
              <Link href={`/diaria/entrada/edit/${row.original.id}`}>
                <DropdownMenuItem>
                  <SquarePen className="mr-2 h-4 w-4" />
                  Editar
                </DropdownMenuItem>
              </Link>
              <Link href={`/diaria/entrada/view/${row.original.id}`}>
                <DropdownMenuItem>
              ...
              <DropdownMenuSeparator />
              <DropdownMenuItem
                onClick={() => setDeletingEntrada(row.original)}
    ...
    <div className="p-4 bg-background shadow-md rounded-lg mt-4">
      <div className="w-full items-center justify-between">
        ...
        <div className="flex flex-col md:flex-row md:items-center md:justify-between gap-4 mb-4">
          ...
          <div className="flex-1 md:w-auto">
            <Input
              placeholder="Nombre Estancia Origen..."
              value={
                (table
                  .getColumn("NombreEstanciaOrigen")
          ...
          <div className="flex flex-col md:flex-row gap-2 w-full md:w-auto">
              ...
              <DropdownMenuContent className="p-2 w-60">
                ...
                <div>
                  ...
                  <Select
                    ...
                    onValueChange={(value) =>
                      handleFilterChange("potrero", value, setSelectedMotivo)
                    }
                  >
                    <SelectTrigger>
                      <SelectValue placeholder="Todos" />
                    ...
                    <SelectContent>
                      ...
                      {[
                        ...new Set(
                          data.map((d) => d.motivo?.nombre).filter(Boolean)
                        ),
                      ].map((motivo) => (
            ...
            {selectedRows.length > 0 && (
              ...
              >
                <Button
                  variant="destructive"
                  onClick={() => setDeletingEntrada(selectedRows)}
                  size="sm"
                  className="w-full md:w-auto"
                >
      ...
      <div className="rounded-md border">
        ...
        <Dialog
          ...
          onOpenChange={(open) => {
            if (!open) setDeletingEntrada(null);
          }}
        >
          <DialogContent>
            <DialogHeader>
            ...
            <DialogDescription>
              {Array.isArray(deletingEntrada) ? (
                <p className="mt-2">
                  ¿Estás seguro de que deseas eliminar los{" "}
                  <span className="font-bold">{deletingEntrada.length}</span>{" "}
                ...
                <p className="mt-2">
                  ...
                  <span className="font-bold italic">
                    {" " + deletingEntrada?.NombreEstanciaOrigen + " "}
                  </span>
            ...
            <DialogFooter className="gap-2 sm:space-x-0">
              <DialogClose>
                <Button
                  ...
                  className="mr-5"
                  variant="outline"
                  onClick={() => setDeletingEntrada(null)}
                >
                  Cancelar
                </Button>
                <Button
